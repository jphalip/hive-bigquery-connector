...
public class HiveCompat extends HiveCompatBase {
    ...
    public Object convertTimeUnitFromArrow(ObjectInspector objectInspector, Object value, int rowId) {
        ...
        if (objectInspector instanceof TimestampLocalTZObjectInspector) {
            TimestampTZ timestampTZ = DatetimeUtils.getHiveTimestampTZFromUTC(((TimeStampMicroTZVector) value).get(rowId));
    ...
    @Override
    public Map<PrimitiveObjectInspector.PrimitiveCategory, StandardSQLTypeName> getHiveToBqTypeMappings() {
        String cacheKey = "Hive3Compat.getHiveToBqTypeMappings";
        Map<PrimitiveObjectInspector.PrimitiveCategory, StandardSQLTypeName> cachedResult = (Map<PrimitiveObjectInspector.PrimitiveCategory, StandardSQLTypeName>) cache.getIfPresent(cacheKey);
        if (cachedResult != null) {
            return cachedResult;
        }
        Map<PrimitiveObjectInspector.PrimitiveCategory, StandardSQLTypeName> map = super.getHiveToBqTypeMappings();
        map.put(PrimitiveObjectInspector.PrimitiveCategory.TIMESTAMPLOCALTZ, StandardSQLTypeName.TIMESTAMP);
        cache.put(cacheKey, map);
        return map;
    }

    @Override
    public Map<PrimitiveObjectInspector.PrimitiveCategory, DescriptorProtos.FieldDescriptorProto.Type> getHiveToProtoMappings() {
        String cacheKey = "Hive3Compat.getHiveToProtoMappings";
        Map<PrimitiveObjectInspector.PrimitiveCategory, DescriptorProtos.FieldDescriptorProto.Type> cachedResult = (Map<PrimitiveObjectInspector.PrimitiveCategory, DescriptorProtos.FieldDescriptorProto.Type>) cache.getIfPresent(cacheKey);
        if (cachedResult != null) {
            return cachedResult;
        }
        Map<PrimitiveObjectInspector.PrimitiveCategory, DescriptorProtos.FieldDescriptorProto.Type> map = super.getHiveToProtoMappings();
        map.put(
